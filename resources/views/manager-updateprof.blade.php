      ...
      <div class="col-md-6">
        ...
        <div class="card shadow-lg">
          <div class="card-header" style="width: 100%; background-color:#0866FF; position: relative;">
            <a href="{{ route('manager-profile') }}" class="btn btn-link position-absolute text-white" style="text-decoration: none; left: 10px;">&lt; Back</a>
            <h3 class="text-white text-center">Update Account</h3>
          </div>
          <div class="card-body">
            <form action="{{ route('manager-updateprof') }}" method="post">
              @csrf
              <div class="mb-3">
                <label for="fullname" class="form-label"><strong>Full Name:</strong></label>
                <input type="text" class="form-control" id="fullname" name="fullname" value="{{ old('fullname', $teammanager->fullname) }}" required>
                @error('fullname')
              ...
              <div class="mb-3">
                ...
                <input type="email" class="form-control" id="email" name="email" value="{{ old('email', $teammanager->email) }}" required>
                @error('email')
                  <div class="text-danger">{{ $message }}</div>
                @enderror
              </div>
              <div class="mb-3">
                ...
                <input type="tel" class="form-control" id="phone_number" name="phone_number" value="{{ old('phone_number', $teammanager->phone_number) }}" required>
                @error('phone_number')
                  <div class="text-danger">{{ $message }}</div>
                @enderror
              </div>
              <div class="mb-3">
                ...
                <input type="tel" class="form-control" id="team_name" name="team_name" value="{{ old('team_name', $teammanager->team_name) }}" required>
                @error('team_name')
                  <div class="text-danger">{{ $message }}</div>
                @enderror
              </div>
